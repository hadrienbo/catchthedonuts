...
0x00, 0x00, 0x77, 0x77, 0x7b, 0x7e, 0xee, 0xe0, 
0x00, 0x07, 0x77, 0x77, 0x77, 0x77, 0xee, 0xe0, 
0x00, 0x07, 0x77, 0x77, 0x7b, 0x7b, 0x7e, 0xee, 
0x00, 0x07, 0x77, 0x77, 0x77, 0x77, 0x77, 0xee, 
0x00, 0x00, 0x77, 0x77, 0x77, 0x77, 0x77, 0xee, 
0x00, 0x00, 0xb7, 0x77, 0xbb, 0x77, 0x7e, 0xee, 
0x00, 0x01, 0xbb, 0xbb, 0x11, 0x77, 0x7e, 0x77, 
0x00, 0x0b, 0x17, 0x71, 0xb7, 0x77, 0x7e, 0x7b, 
0x00, 0x77, 0x77, 0x77, 0x77, 0x77, 0x77, 0xe7, 
0x00, 0x77, 0x77, 0x77, 0x77, 0x77, 0x77, 0x77, 
0x07, 0x77, 0x77, 0x77, 0x77, 0x77, 0x77, 0x70, 
0x07, 0x70, 0x07, 0x77, 0x71, 0x17, 0x77, 0x70, 
0x77, 0x00, 0x00, 0x71, 0x17, 0x77, 0x77, 0x70, 
0x70, 0x00, 0x00, 0x01, 0x77, 0x00, 0x77, 0x00, 
...
{
	if((	spritegetvalue(n, S_X) < 0 && spritegetvalue(n, S_SPEEDX) < 0) || (	spritegetvalue(n, S_X) > 127 && spritegetvalue(n, S_SPEEDX) > 0))
	{		
		// Si le sprite est hors de l'ecran, du coté opposé ou il a commence.
		// Le sprite repop du meme cote qu'homer, a peu pres sur sa position.
		
		spritesetvalue(n, S_Y, y * 4 + random(30) - 15);
		if(spritegetvalue(n, S_Y) < 0) spritesetvalue(n, S_Y, 0);
		else if(spritegetvalue(n, S_Y) > 112) spritesetvalue(n, S_Y, 112);
		
		if(x > 16)
		{
			
			spritesetvalue(n, S_X, 136);	
			spritesetvalue(n, S_FLIP_HORIZONTAL, 0);
			spritesetvalue(n, S_SPEEDX,  -8 - level - (level * random(3)));
		} 	
		else
		{
			spritesetvalue(n, S_X, -20);
			spritesetvalue(n, S_FLIP_HORIZONTAL, 1);	
			spritesetvalue(n, S_SPEEDX,  8 + level + (level * random(3)));
		}		
	}
}
